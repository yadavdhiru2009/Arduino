...
void INA226_DP::reset_INA226(){
    writeRegister(INA226_CONF_REG, INA226_RST); 
}

void INA226_DP::setCorrectionFactor(float corr){
    corrFactor = corr;
    calVal *= corrFactor;
    writeRegister(INA226_CAL_REG, calVal);
}

void INA226_DP::setAverage(INA226_AVERAGES averages){
...
void INA226_DP::setConversionTime(INA226_CONV_TIME shuntConvTime, INA226_CONV_TIME busConvTime){
    uint16_t currentConfReg = readRegister(INA226_CONF_REG);
...
void INA226_DP::setConversionTime(INA226_CONV_TIME convTime){
    setConversionTime(convTime, convTime);
}

void INA226_DP::setMeasureMode(INA226_MEASURE_MODE mode){
...
void INA226_DP::setCurrentRange(INA226_CURRENT_RANGE range){
...
void INA226_DP::setResistorRange(float resistor, float current_range){
...
void INA226_DP::setCalVal(float calib, float current_range){
    float current_LSB=current_range/32768.0;
...
float INA226_DP::getShuntVoltage_V(){
    int16_t val;
    val = static_cast<int16_t>(readRegister(INA226_SHUNT_REG));
    return (val * 0.0000025 * corrFactor);  
}

float INA226_DP::getShuntVoltage_mV(){
    int16_t val;
    val = static_cast<int16_t>(readRegister(INA226_SHUNT_REG));
    return (val * 0.0025 * corrFactor); 
}

float INA226_DP::getBusVoltage_V(){
    uint16_t val;
    val = readRegister(INA226_BUS_REG);
    return (val * 0.00125);
}

uint16_t INA226_DP::getRawBusVoltage_V(){
    uint16_t val;
    val = readRegister(INA226_BUS_REG);
    return (val);
}

float INA226_DP::getCurrent_mA(){
    int16_t val;
    val = static_cast<int16_t>(readRegister(INA226_CURRENT_REG));
    return (val / currentDivider_mA);
}

uint16_t INA226_DP::getRawCurrent_mA(){
    int16_t val;
    val = static_cast<int16_t>(readRegister(INA226_CURRENT_REG));
    return (val);
}

float INA226_DP::convertBusVoltage_V(uint16_t value){
    return (value * 0.00125);
}

float INA226_DP::convertCurrent_mA(uint16_t value){
    return (value / currentDivider_mA);
}


float INA226_DP::getCurrent_A() {
    return (getCurrent_mA()/1000);
}

float INA226_DP::getBusPower(){
    uint16_t val;
    val = readRegister(INA226_PWR_REG);
    return (val * pwrMultiplier_mW);
}

void INA226_DP::startSingleMeasurement(){
...
void INA226_DP::setAlertPinActiveHigh(){
    uint16_t val = readRegister(INA226_MASK_EN_REG);
    val |= 0x0002;
    writeRegister(INA226_MASK_EN_REG, val);
}

void INA226_DP::enableAlertLatch(){
    uint16_t val = readRegister(INA226_MASK_EN_REG);
    val |= 0x0001;
    writeRegister(INA226_MASK_EN_REG, val);
}

void INA226_DP::enableConvReadyAlert(){
    uint16_t val = readRegister(INA226_MASK_EN_REG);
    val |= 0x0400;
    writeRegister(INA226_MASK_EN_REG, val);
}
    
void INA226_DP::setAlertType(INA226_ALERT_TYPE type, float limit){
...
void INA226_DP::readAndClearFlags(){
    uint16_t value = readRegister(INA226_MASK_EN_REG);
...
void INA226_DP::writeRegister(uint8_t reg, uint16_t val){
  _wire->beginTransmission(i2cAddress);
...
uint16_t INA226_DP::readRegister(uint8_t reg){
